...
// 1–2. Capacitor config — load it under release env and verify the result
//      has no server.url. We use `tsx` if available, else fall back to a
//      regex-based check on the compiled native config (capacitor.config.json
//      is generated by `npx cap copy`).
...
if (!existsSync(capPath)) {
  ...
  check(
    "capacitor.config.ts: env-aware (no hardcoded server.url at module top level)",
    /isProduction\s*\?/.test(cap) || !/server\s*:\s*\{[^}]*url\s*:\s*["'`]https?:/.test(cap),
    "Make `server.url` conditional on CAP_ENV/NODE_ENV so release builds drop it.",
  );

  // Verify the native-side generated config (capacitor.config.json) — if the
  // user has run `npx cap sync` / `npx cap copy` recently, this file reflects
  // the *actual* config the iOS/Android shells will use.
  for (const platform of ["ios", "android"]) {
    const nativeJson = join(ROOT, platform, platform === "ios" ? "App/App/capacitor.config.json" : "app/src/main/assets/capacitor.config.json");
    if (existsSync(nativeJson)) {
      try {
        const parsed = JSON.parse(readFileSync(nativeJson, "utf8"));
        const hasUrl = parsed?.server?.url;
        check(
          `${platform}: native capacitor.config.json has no server.url`,
          !hasUrl,
          `Found server.url=${hasUrl} in ${nativeJson}. Re-run \`CAP_ENV=production npx cap sync ${platform}\`.`,
        );
      } catch {
        warnings.push(`Could not parse ${nativeJson}`);
      }
    }
  }
}
